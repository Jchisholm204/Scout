...
StaticQueue_t lidar_rx_sqh;
struct udev_pkt_lidar lidar_rx_buf[USBI_LIDAR_BUF_SIZE] = {0};
...
CtrlQueueHndl_t lidar_tsk_init(struct lidar_tsk *const pHndl,
    ...
    pHndl->tsk.hndl = xTaskCreateStatic(vLidarTsk,
                                        "gplan",
                                        LIDAR_TSK_STACK_SIZE,
                                        pHndl,
    ...
    rplidar_init(&pHndl->rplidar[eLidarFront], eLidarFront, false, port_lidar_front, pHndl->lidar_rx);
    rplidar_init(&pHndl->rplidar[eLidarVertical], eLidarVertical, false, port_lidar_vertical, pHndl->lidar_rx);
...
void vLidarTsk(void *pvParams) {
    struct lidar_tsk *pHndl = (struct lidar_tsk *) pvParams;
