...
static inline void hal_uart_init(USART_TypeDef *uart,
                                 ...
                                 uint16_t pin_tx,
                                 uint16_t pin_rx) {
    // figure 19. selecting an alternate function (7=spi2/3, usart1..3, uart5,
    // spdif-in)
    ...
    gpio_set_af(pin_tx,
                (enum GPIO_AF_MODE)(
                    (uart == UART4 || uart == UART5 || uart == USART6) ? 8
                                                                       : 7));
    ...
    if (uart == USART1 || uart == USART6) {
        // uart->BRR = APB2_FREQUENCY / baud; // FREQ is a UART bus frequency
        uart->BRR = (APB2_FREQUENCY + (baud / 2)) / baud;
    } else {
...
static inline uint32_t hal_uart_read_status(const USART_TypeDef *uart) {
    return ((uint32_t) uart->SR);
}
...
static inline void hal_uart_write_buf(USART_TypeDef *uart,
                                      char *buf,
                                      size_t len) {
    while (len-- > 0)
