 ...
 *
 * Note about clocks:
 *  The CMSIS "SystemCoreClock" does not work. This is because the STM32 eval
 * boards contain a 25Mhz external crystal oscillator. Our FSAE boards use an
 * 8Mhz crystal. Therefore make sure to use the correct clock in your design.
 *
...
static inline void hal_clock_flash_init(void) {
    ...
    SET_BIT(FLASH->ACR,
            (FLASH_ACR_LATENCY_5WS | FLASH_ACR_PRFTEN | FLASH_ACR_ICEN));
}
...
static inline void hal_clock_osc_init(void) {
    ...
    WRITE_REG(RCC->PLLCFGR,
              RCC_PLLCFGR_PLLSRC_HSE | PLL_M | PLL_N << RCC_PLLCFGR_PLLN_Pos |
...
static inline void hal_clock_init(void) {
    ...
    MODIFY_REG(RCC->CFGR, RCC_CFGR_PPRE2, (RCC_CFGR_HPRE_DIV16));
    // Setup the HCLK
